(function() {
    ...
    YAHOO.util.GridBuilder = {
        ...
        getCode: function(lorem) {
            ...
            code += 'var templates = '+YAHOO.lang.JSON.stringify(this.templates.getValue())+';\n';
            code += 'YAHOO.util.Event.onDOMReady(function () {\n';
            code += '   var els = YAHOO.util.Dom.getElementsByClassName(\'yourData\');\n';
            code += '   for(var i = 0 ; i < templates.length ; i++) {\n';
            code += '      els[i].innerHTML = "<img src=\'http://yui.yahooapis.com/' + YAHOO.VERSION + '/build/assets/skins/sam/ajax-loader.gif\'/>";\n';
            code += '   }\n';
            code += '   YQL.queryCode( yqlCode, function(o) {\n';
            code += '      for(var i = 0 ; i < templates.length ; i++) {\n';
            code += '         var tpl = TrimPath.parseTemplate( templates[i] );\n';
            code += '         var templateResult = tpl.process(o.query.results);\n';
            code += '         els[i].innerHTML = templateResult;\n';
            code += '      }\n';
            code += '   });\n';
            code += '});\n';
            code += '</script>\n';
        ...
        runYQL: function() {
          ...
          this.jsontreeContainer.innerHTML = "<img src='http://yui.yahooapis.com/2.7.0/build/assets/skins/sam/ajax-loader.gif'/>";
          var that= this;
